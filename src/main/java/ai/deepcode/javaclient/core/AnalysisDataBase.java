...
public abstract class AnalysisDataBase {
    ...
    public void updateCachedResultsForFiles(
        ...
        Set<Object> filesToRemove = mapProject2RemovedFiles.remove(project);
        if (filesToRemove == null) filesToRemove = Collections.emptySet();
        ...
        // TODO clarify why this is even done
        final List<String> first50FilesName =
                allProjectFiles.stream().limit(50).map(pdUtils::getFileName).collect(Collectors.toList());
        dcLogger.logInfo("Update requested for " + allProjectFiles.size() + " files: " + first50FilesName);
        ...
        try {
            ...
            // filepath: (hash : file)
            Map<String, FileHash2ContentRequest> filesToProceed = convertToMap(allProjectFiles);

            if (!filesToProceed.isEmpty()) {
                // collection already checked to be not empty
                final Object firstFile = filesToProceed.keySet().iterator().next();
                dcLogger.logInfo("Files to proceed (not found in cache): " + filesToProceed.size() + "\nHash for first file "
                        + pdUtils.getFileName(firstFile) + " [" + hashContentUtils.getHash(firstFile) + "]");

            } else if (!filesToRemove.isEmpty()) {
                dcLogger.logWarn("Nothing to update for " + allProjectFiles.size() + " files: " + allProjectFiles);
            }
    ...
    @NotNull
    private Map<String, FileHash2ContentRequest> convertToMap(@NotNull Collection<Object> allProjectFiles) {
        return
                allProjectFiles.stream()
                        .filter(Objects::nonNull)
                        .filter(file -> !mapFile2Suggestions.containsKey(file))
                        .collect(Collectors.toMap(
                                pdUtils::getDeepCodedFilePath,
                                file -> new FileHash2ContentRequest(
                                        hashContentUtils.getHash(file),
                                        hashContentUtils.getFileContent(file))
                        ));
    }
    ...
    private Map<Object, List<SuggestionForFile>> retrieveSuggestions(
            @NotNull Object project,
            @NotNull Map<String, FileHash2ContentRequest> filesToProceed,
            @NotNull Set<Object> filesToRemove,
            @NotNull Object progress) {
        ...
        List<String> filesToAnalyse =
                filesToProceed.keySet().stream().collect(Collectors.toList());
    ...
    private List<String> createBundleStep(
        ...
        for (Object file : filesToProceed.keySet()) {
            hashContentUtils.removeFileHashContent(file);
            pdUtils.progressCheckCanceled(progress);
    ...
    private void uploadFiles(
            @NotNull Object project,
            @NotNull Map<String, FileHash2ContentRequest> filesToProcess,
            @NotNull List<String> missingFiles,
        ...
        Map<String, FileHash2ContentRequest> filesChunk = new HashMap<>(totalFiles);
        for (String filePath : missingFiles) {
            fileCounter = updateProgress(progress, fileCounter, totalFiles);

            FileHash2ContentRequest file = filesToProcess.get(filePath);
            if (file == null) {
                if (brokenMissingFilesCount == 0) {
                    brokenMissingFilesMessage =
                            " files requested in missingFiles not found in filesToProcess (skipped to upload)."
            ...
            final long fileSize = file.getContent().length();
            ...
            filesChunk.put(filePath,
                    new FileHash2ContentRequest(hashContentUtils.getHash(file), hashContentUtils.getFileContent(file)));
        }
        if (brokenMissingFilesCount > 0)
            dcLogger.logWarn(brokenMissingFilesCount + brokenMissingFilesMessage);
        dcLogger.logInfo("Last filesToProcess-chunk size: " + fileChunkSize);
        doUploadFiles(project, filesChunk, bundleId, progress);
    }

    private int updateProgress(@NotNull Object progress, int fileCounter, int totalFiles) {
        pdUtils.progressCheckCanceled(progress);
        pdUtils.progressSetFraction(progress, ((double) fileCounter++) / totalFiles);
        pdUtils.progressSetText(
    ...
    private CreateBundleResponse makeNewBundle(
        ...
        logWarningIfExtending(project, mapPath2Hash, filesToRemove, parentBundleId);

        List<String> removedFiles = filesToRemove.stream().map(pdUtils::getDeepCodedFilePath).collect(Collectors.toList());

        logCreation(mapPath2Hash, parentBundleId, removedFiles);
        ...
        if (parentBundleId.isEmpty())
            bundleResponse = DeepCodeRestApi.createBundle(deepCodeParams.getSessionToken(), fileHashRequest);
        else {
            bundleResponse = DeepCodeRestApi.extendBundle(deepCodeParams.getSessionToken(), parentBundleId, new ExtendBundleRequest(fileHashRequest.getFiles(), removedFiles));
        }
    ...
    private void logCreation(@NotNull Map<String, String> mapPath2Hash, String parentBundleId, List<String> removedFiles) {
        String message =
    ...
    private void logWarningIfExtending(@NotNull Object project, @NotNull Map<String, String> mapPath2Hash, @NotNull Collection<Object> filesToRemove, String parentBundleId) {
        if (!parentBundleId.isEmpty()
    ...
    private void doUploadFiles(
            @NotNull Object project,
            @NotNull Map<String, FileHash2ContentRequest> psiFiles,
            @NotNull String bundleId,
            @NotNull Object progress) {
        dcLogger.logInfo("Uploading " + psiFiles.size() + " files... ");
        if (psiFiles.isEmpty()) return;
        ExtendBundleWithContentRequest req = new ExtendBundleWithContentRequest(psiFiles, Collections.emptyList());

        @NotNull EmptyResponse uploadFilesResponse =
                DeepCodeRestApi.extendBundle(deepCodeParams.getSessionToken(), bundleId, req);
    ...
    private Map<Object, List<SuggestionForFile>> parseGetAnalysisResponse(
            @NotNull Object project,
            @NotNull Map<String, FileHash2ContentRequest> files,
            GetAnalysisResponse response,
            @NotNull Object progress) {
        Map<Object, List<SuggestionForFile>> result = new HashMap<>();
